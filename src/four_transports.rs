#![forbid(unsafe_code)]
//! Serving the four avenues from `act_api_server`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `act-lib-core` |
...
pub const SERVICE_SLUG: &str = "act";

/// Environment prefix for every variable this service reads.
pub const ENV_PREFIX: &str = "ACT";
...
///
/// Bind `ACT_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `ACT_NATS_URL`.
///
/// # Errors
