//! Four-avenue web ↔ API binding for `anticaptrad`.
//!
//! 1. Direct read-only DB via `*-lib-core` named queries (no migrations).
//! 2. Stateless HTTP from `app.anticaptrad.dev` to `api.anticaptrad.dev`.
//! 3. Stateful TLS 1.3/mTLS TCP to `api.anticaptrad.dev:7443`.
...
pub const GITHUB_ORG: &str = "anticaptrad";
pub const ORG_SLUG: &str = "anticaptrad";
pub const DNS_ZONE: &str = "anticaptrad.dev";
