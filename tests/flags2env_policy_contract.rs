...
fn policy() -> String { fs::read_to_string(root().join(".cli-flags.toml")).expect("read policy") }
...
fn policy_uses_aliases_not_retired_long_keys() {
    let text = policy();
    assert!(!text.lines().any(|line| line.trim_start().starts_with("long =")));
...
fn sensitive_values_remain_environment_only() {
    let text = policy();
    for key in ["ACT_API_TLS_KEY_FILE", "ACT_NATS_OPERATION_HMAC_KEY", "ACT_OPERATION_DATABASE_URL", "NATS_URL", "SHARED_AUTH_SERVICE_CREDENTIAL", "YOUTUBE_GAS_API_KEY"] {
